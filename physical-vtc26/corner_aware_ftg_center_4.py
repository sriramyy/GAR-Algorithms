...
class ArcFollowGap(Node):
    def __init__(self):
        ...
        self.HORIZON_BASE = 3.5          # m of arc we look ahead. THE turn-in knob:
        #                             larger -> earlier, smoother turn-in.
        self.HORIZON = self.HORIZON_BASE
        self.HORIZON_GAIN = 0.35
        self.HORIZON_MAX = 8.0
        self.SWEEP_MAX = math.pi    # never evaluate more than 180 deg of arc
        self.INFLATE_BASE = 0.26         # half corridor the arc must keep clear (m)
        self.INFLATE = self.INFLATE_BASE
        self.INFLATE_GAIN = 0.010
        ...
        self.W_CENTER = 0.18        # <-- pulls the chosen arc toward the
        #                                track centerline (uses instantaneous
        #                                left/right lidar balance). Raise if
        #                                still hugging a wall, lower if it
        #                                fights corner entry.

        self.KAPPA_ALPHA = 0.60    # light LPF on the chosen curvature
        ...
        self.A_BRAKE = 5.0          # m/s^2 used for look-ahead braking
        ...
        # Avoid the 1/0 in R = 1/kappa WITHOUT collapsing the near-zero
        # curvatures onto a single positive epsilon (that used to bias
        # every "drive straight" evaluation slightly left, which integrates
        # into steady wall-hugging over a straightaway).
        eps = 1e-3
        k = np.where(np.abs(k) < eps, np.where(k >= 0, eps, -eps), k)
        self.kappas = k
        self.R = (1.0 / k).reshape(-1, 1)
        # self.s_eval = np.minimum(self.HORIZON, self.SWEEP_MAX / np.abs(k))
        self.sweep_cap = self.SWEEP_MAX / np.abs(k)
        ...
        self.get_logger().info("ArcFollowGap started (curvature-space planner) 3.")
    ...
    def scan_to_points(self, scan: LaserScan):
        ...
        # NEW: suppress short-duration wall bulges/pockets. A genuine wide
        # turn stays wide over a large angular span and survives this; a
        # narrow bulge gets pulled back down to the tighter wall around it.
        win = 7  # tune: beams, not degrees -- depends on angle_increment
        r = np.array([r[max(0, i-win):i+win+1].min() if r[max(0,i-win):i+win+1].size else r[i]
    ...
    def plan(self, X, Y):
        ...
        self.HORIZON = float(np.clip(self.HORIZON_BASE + self.HORIZON_GAIN * self.ego_speed,
                                  self.HORIZON_BASE, self.HORIZON_MAX))
        self.s_eval = np.minimum(self.HORIZON, self.sweep_cap)

        self.INFLATE = float(np.clip(self.INFLATE_BASE + self.INFLATE_GAIN * self.ego_speed,
                                  self.INFLATE_BASE, 0.45))
        ...
        # centerline bias: once an arc already clears CLEAR_REF the pure
        # clearance term goes flat (clear_n saturates at 1.0), so nothing
        # pulls the car back toward the middle of the corridor. Compute an
        # instantaneous left/right room imbalance from the raw scan and
        # penalize curvature that steers further INTO the side that already
        # has less room (kn > 0 is left-curving in this frame).
        if self.ranges is not None and self.angles is not None:
            left_mask = self.angles > 0
            right_mask = self.angles < 0
            left_d = float(np.min(self.ranges[left_mask])) if left_mask.any() else self.RANGE_MAX
            right_d = float(np.min(self.ranges[right_mask])) if right_mask.any() else self.RANGE_MAX
            denom = max(left_d + right_d, 1e-3)
            center_bias = (left_d - right_d) / denom   # >0 => more room on left
            J += self.W_CENTER * center_bias * kn
